...
async function getPromotionDiscount(hotelId: string): Promise<number> {
  try {
    ...
    const platformDiscount = platformPromotion
      ? parseFloat(platformPromotion.value)
      : 0;
  ...
  try {
    ...
    const result = await db
      .select({
        ...
        lowestBasePrice: sql<number>`MIN(${rooms.basePrice})`.as(
          "lowestBasePrice"
        ),
      })
      .from(hotels)
    ...
    const activePromotions =
      hotelIds.length > 0
        ? await db.query.promotions.findMany({
            where: and(
              sql`${promotions.hotelId} IN (${sql.join(
                hotelIds.map((id: string) => sql`${id}`),
                sql`, `
              )})`,
              eq(promotions.isActive, true)
            ),
    ...
    if (latitude && longitude) {
      filtered = filtered.filter(
        (h: (typeof filtered)[number]) =>
          h.distance !== undefined && h.distance <= radiusKm
      );
    }
    ...
    if (minPrice !== undefined) {
      filtered = filtered.filter(
        (h: (typeof filtered)[number]) => h.lowestPrice >= minPrice
      );
    }
    if (maxPrice !== undefined) {
      filtered = filtered.filter(
        (h: (typeof filtered)[number]) => h.lowestPrice <= maxPrice
      );
    }
    if (payAtHotel) {
      filtered = filtered.filter(
        (h: (typeof filtered)[number]) => h.payAtHotel === true
      );
    }
    ...
    if (minRating !== undefined && minRating > 0) {
      filtered = filtered.filter(
        (h: (typeof filtered)[number]) => h.rating >= minRating
      );
    }
...
export async function getHotelById(hotelId: string) {
  try {
    ...
    // Calculate lowestPrice from lowestDynamicPrice or min room price
    let lowestPrice = 0;
    ...
    return {
      ...
      lowestPrice, // Add lowestPrice for mobile app
      imageUrl: hotel.coverImage, // Add imageUrl field
    };
  } catch (error) {
  ...
  try {
    ...
    for (const group of roomGroups.values()) {
      ...
      for (const dateStr of stayDates) {
        ...
        let priceEntry = null;
        for (const roomId of group.availableRoomIds.length > 0
          ? group.availableRoomIds
          : [representativeRoom.id]) {
          priceEntry = inventoryPrices.find(
            (p: (typeof inventoryPrices)[number]) =>
              p.roomId === roomId && p.date === dateStr
          );
      ...
      if (promotionDiscount > 0) {
        ...
        finalTotalPrice = applyPromotionDiscount(
          totalDynamicPrice,
          promotionDiscount
        );
      }
      ...
      groupedRooms.push({
        ...
        unavailableReason:
          availableCount === 0 ? "All rooms booked for these dates" : undefined,
